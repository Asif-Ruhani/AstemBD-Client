...
const fetchHscCourses = async () => {
  const res = await fetch('https://astembd-server.onrender.com/hsc-courses');
  if (!res.ok) {
    throw new Error(`Failed to fetch HSC courses: ${res.status}`);
  }
...
const HSC = () => {
  const {
    ...
    queryKey: ['hsc-courses'],
    queryFn: fetchHscCourses,
    ...
    <section className="bg-slate-50/70 dark:bg-zinc-950 py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center text-rose-500 font-medium">
            {error?.message || 'Failed to load HSC courses.'}
          </div>
        ) : (
