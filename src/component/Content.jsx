      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {cards.map((card, index) => {
            ...
            return (
              <div
                ...
                className={`group relative flex flex-col justify-between bg-white dark:bg-[#0E131F] rounded-3xl border shadow-[0_4px_20px_-4px_rgba(15,23,42,0.05)] hover:shadow-[0_20px_40px_-15px_rgba(15,23,42,0.12)] dark:hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.6)] hover:-translate-y-1.5 transition-all duration-300 ease-out overflow-hidden ${isUnlocked
                    ? 'border-slate-200/90 dark:border-white hover:border-black dark:hover:border-white'
                    : 'border-slate-200/90 dark:border-white hover:border-black dark:hover:border-white'
                  }`}
              >
                ...
                <div className="p-6 sm:p-7 pt-0 relative z-10">
                  <Link
                    ...
                    className={`group/btn relative w-full inline-flex items-center justify-center gap-2 py-3 px-5 rounded-2xl text-xs sm:text-sm font-semibold transition-all duration-200 active:scale-[0.98] shadow-sm cursor-pointer ${isUnlocked
                        ? 'text-black bg-white hover:bg-gray-300 border border-black'
                        : 'text-black bg-white hover:bg-gray-300 border border-black'
                      }`}
                  >
                    <span className="relative z-10 transition-transform duration-200 group-hover/btn:-translate-x-0.5">
                      {isUnlocked ? 'Explore' : 'View Course & Preview'}
                    </span>
                    <svg
            ...
            //     key={courseIdentifier}
            //     className={`group relative flex flex-col justify-between bg-white dark:bg-[#0E131F] rounded-3xl border shadow-[0_4px_20px_-4px_rgba(15,23,42,0.05)] hover:shadow-[0_20px_40px_-15px_rgba(15,23,42,0.12)] dark:hover:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.6)] hover:-translate-y-1.5 transition-all duration-300 ease-out overflow-hidden ${isUnlocked
            //         ? 'border-slate-200/90 dark:border-zinc-800/80 hover:border-black dark:hover:border-white'
            //         : 'border-slate-200/90 dark:border-zinc-800/80 hover:border-black dark:hover:border-white'
            //       }`}
            //   >
            //     <div className="absolute top-0 right-0 -mr-16 -mt-16 w-36 h-36 bg-gradient-to-br from-indigo-500/10 to-emerald-500/10 dark:from-indigo-400/10 dark:to-emerald-400/10 rounded-full blur-2xl group-hover:scale-150 transition-transform duration-500 pointer-events-none" />
            ...
            //       {/* ২ কলাম বিশিষ্ট ফিচার সেকশন */}
            //       <div className="mt-auto grid grid-cols-2 gap-x-3 gap-y-2 pt-4 border-t border-slate-100 dark:border-zinc-800/80">
            //         {card.features?.map((feature, idx) => (
            ...
            //               </svg>
            //             </div>
            //             <span className="leading-snug truncate" title={feature}>{feature}</span>
            //           </div>
            //         ))}
            //       </div>
            //     </div>
            ...
            //       <Link
            //         to={targetCourseRoute}
            //         className={`group/btn relative w-full inline-flex items-center justify-center gap-2 py-2.5 px-5 rounded-2xl text-xs sm:text-sm font-semibold transition-all duration-200 active:scale-[0.98] shadow-sm cursor-pointer ${isUnlocked
            //             ? 'text-black bg-white hover:bg-gray-300 border border-black'
            //             : 'text-black bg-white hover:bg-gray-300 border border-black'
            //           }`}
            //       >
