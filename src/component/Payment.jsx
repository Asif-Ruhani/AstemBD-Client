...
// import useAxiosSecure from '../Hooks/useAxiosSecure'; // Using only axiosSecure
...
//                 text: err.response?.data?.message || err.message || 'Failed to submit payment details'
//             });
//         } finally {
...
const Payment = () => {
    ...
    // Enforce login requirement: redirect unauthenticated users
    useEffect(() => {
        if (!authLoading && !user) {
            Swal.fire({
                icon: 'warning',
                title: 'Authentication Required',
                text: 'You must be logged in to submit payment details.',
                confirmButtonColor: '#0f172a'
            }).then(() => {
    ...
    // Populate and lock verified email from auth state
    useEffect(() => {
        if (user?.email) {
            setValue('email', user.email);
        }
    ...
    // Submit payment verification strictly under verified user email
    const onSubmit = async (formData) => {
        if (!user?.email) {
            Swal.fire({
                icon: 'error',
        ...
        try {
            ...
            Swal.fire({
                ...
                text: 'Failed to submit payment details, for any help, please contact +8801783717408'
            });
        } finally {
            setIsSubmitting(false);
        }
            ...
            <div className="w-full max-w-2xl bg-white dark:bg-zinc-900 rounded-3xl border border-slate-200/80 dark:border-zinc-800 shadow-xl overflow-hidden p-6 sm:p-10 relative">
                ...
                <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
                    ...
                    <div>
                        <label className="block text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-zinc-400 mb-1.5">
                            {t.form?.emailLabel}{' '}
                            <span className="text-slate-400 text-[10px] font-normal lowercase">(linked to your account)</span>
                        </label>
                        <input
                            type="email"
                            readOnly
                            {...register('email')}
                            className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-zinc-800 bg-slate-100 dark:bg-zinc-800/60 text-slate-500 dark:text-zinc-400 font-medium text-sm cursor-not-allowed select-none focus:outline-none"
                        />
                    </div>
