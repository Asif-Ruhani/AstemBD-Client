...
const BasicEngVocab = () => {
    ...
    const {
        ...
        queryKey: ['eng-vocab-courses', user?.uid],
        queryFn: async () => {
            const res = await axiosSecure.get('/basic-eng-vocab');
            return res.data?.courses || res.data?.sections || res.data || [];
        },
    ...
    if (isError) {
        return (
            <div className="py-20 text-center text-rose-500 font-medium">
                Failed to load tracks: {error?.message || 'Something went wrong.'}
            </div>
        );
    }
    ...
    return (
        <section className="bg-slate-50/70 dark:bg-zinc-950 py-20 px-4 sm:px-6 lg:px-8">
            {/* Container constrained to max-w-3xl for the 2-column grid */}
            <div className="max-w-7xl mx-auto">
                ...
                {/* Column Responsive Grid Style */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl: gap-6">
                    {cards.map((card) => (
                        <div
                        ...
                        >
                            ...
                            <div className="p-6 flex flex-col flex-1">
                                ...
                                <div className="flex items-center justify-between mb-5">
                                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${card.accentBg || 'bg-slate-100 text-slate-800'} shadow-sm ring-1 ring-black/5`}>
                                        {card.iconSvg ? (
