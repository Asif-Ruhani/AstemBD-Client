...
const EverydayWords = () => {
  ...
  // TanStack Query handles fetching, caching, and token timing cleanly
  const {
    data: sections = [],
    isLoading,
    isError,
    ...
    queryFn: async () => {
      const token = await user.getIdToken();
      const response = await fetch('https://astembd-server.onrender.com/sections', {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
    ...
    staleTime: 1000 * 60 * 10, // Caches data for 10 mins so returning to this page is instant
  });
