...
//   // 2. Check if this is an extra section
//   const isExtra = Boolean(code);
//   const activeId = code || sectionNumber;
...
//     isError,
//   } = useQuery({
//     queryKey: ["sectionDetail", isExtra ? "extra" : "regular", activeId, user?.uid],
//     queryFn: async ({ signal }) => {
//       // 3. Switch endpoint based on which page the user opened
//       const url = isExtra
//         ? `/everydayWordExtraSectionDetail/${activeId}`
//         : `/everydayWordSectionDetail/${activeId}`;

//       const response = await axiosSecure.get(url, { signal });
//       return response.data;
//     },
//     enabled: !authLoading && !!user && !!activeId,
//   });
...
//                   Section {sectionData?.sectionNumber ?? sectionData?.code ?? "--"}
//                 </div>
...
//                 const isExpanded = expandedCard === index;
//                 const term = item.term || item.word || item.phrase;
...
//                       {sectionData?.sectionNumber ?? sectionData?.code}.
//                     </p>
//                   </div>
//                 )}
//             </div>
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              {tab.count !== undefined && tab.count !== null && (
                <span
                  className={`text-xs sm:text-sm font-bold px-2 py-0.5 rounded-full transition-colors leading-normal ${
                    isActive
                      ? "bg-indigo-50 text-indigo-600 dark:bg-indigo-950/80 dark:text-indigo-300 ring-1 ring-indigo-500/20"
                      : "bg-slate-300/60 text-slate-700 dark:bg-zinc-700/60 dark:text-zinc-300 group-hover:bg-slate-300 dark:group-hover:bg-zinc-700"
                  }`}
              ...
              <div className="p-3.5 sm:p-4">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    onClick={() => toggleDetail(index)}
                    aria-label={isExpanded ? "Collapse verb details" : "View verb details"}
                    className={`text-sm font-semibold px-2.5 py-1 rounded-lg transition-colors shrink-0 ${
                      isExpanded
                        ? "bg-rose-50 text-rose-600 hover:bg-rose-100 dark:bg-rose-950/50 dark:text-rose-300"
                        : "bg-slate-100 text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700"
                    }`}
            ...
            >
              ...
              <div className="p-3.5 sm:p-4 flex flex-col justify-between flex-1">
                <div>
                  ...
                  <div className="mt-1 min-h-[2.75rem]">
                    <p
                      className={`text-sm font-medium text-slate-600 dark:text-zinc-300 leading-snug ${
