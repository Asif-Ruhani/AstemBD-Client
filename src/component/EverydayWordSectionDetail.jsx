...
//   // 1. Fetch section list from backend
//   const {
//     data: sectionsList = [],
...
//       const response = await axiosSecure.get("/sections", { signal });
...
//     isError,
//   } = useQuery({
//     queryKey: ["sectionDetail", String(sectionNumber), user?.uid],
//     queryFn: async ({ signal }) => {
...
//   // Dynamic Tabs Definition mapped from JSON fields
...
//     { id: "practice", label: "Practice", key: "morePractices" },
//   ];

//   // Dynamically include tabs only if they contain items in the database response
//   const tabs = TAB_DEFINITIONS.filter((tab) => {
//     const dataField = sectionData?.[tab.key] || (tab.key === "collocations" && sectionData?.collocation);
...
//   // Auto-switch to the first valid tab if the current activeTab doesn't exist in this section
//   useEffect(() => {
...
//     (sectionData?.collocations?.length || sectionData?.collocation?.length || 0);
...
//                   const sectionId = sec.sectionNumber ?? sec.code ?? sec.theme?.code ?? idx + 1;
//                   const sectionTitle = sec.title ?? sec.theme?.title ?? `Section ${sectionId}`;
//                   const sectionIcon = sec.icon ?? sec.theme?.icon ?? "📖";
//                   const isSelected = String(sectionNumber) === String(sectionId);
...
//                         const forms = verbItem.verbForms || {};
//                         const term = verbItem.term;
...
//                   {/* Nouns, Adjectives, Adverbs, and Collocations Tabs */}
//                   {["nouns", "adjectives", "adverbs", "collocations"].includes(activeTab) && (
//                     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 items-stretch">
//                       {(sectionData?.[activeTab] || (activeTab === "collocations" && sectionData?.collocation))?.map((item, index) => {
//                         const isExpanded = expandedCard === index;
...
//                         !(activeTab === "collocations" && sectionData?.collocation?.length > 0) && (
...
//                             <p className="text-sm sm:text-base text-slate-400 dark:text-zinc-500">
...
const EverydayWordSectionDetail = () => {
  ...
  const { user, loading: authLoading, authStatus, hasAccess, hasBundleAccess } = useAuth();
  const axiosSecure = useAxiosSecure();

  // Check if current target is Section 1 (Free Preview) or user holds active entitlement
  ...
  const isEnrolled = useMemo(() => {
    ...
    // 3. Single course direct ID access
  ...
  // 1. Fetch section list for the sidebar
  const {
    data: sectionsList = [],
    isLoading: isSectionsLoading,
  } = useQuery({
    queryKey: ["extraVocabSections"],
    queryFn: async ({ signal }) => {
      const response = await axiosSecure.get("/sections", { signal });
      const raw = response.data;
      if (Array.isArray(raw)) return raw;
  ...
  // 2. Fetch specific section details (Enabled for Section 1 or Enrolled users)
  const {
    data: sectionData = {},
    isLoading,
    isError,
    ...
    enabled: canViewCurrentSection && !!sectionNumber,
    staleTime: 1000 * 60 * 10,
  });
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 [scrollbar-width:thin]">
              {isSectionsLoading ? (
                ...
                sectionsList.map((sec, idx) => {
                  ...
                  const sectionIcon = sec.icon ?? sec.theme?.icon ?? "📖";
                  const isSelected = String(sectionNumber) === String(sectionId);
                  const isSecUnlocked = String(sectionId) === "1" || isEnrolled;
              ...
              <div className="max-w-2xl mx-auto py-24 px-4 text-center space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm sm:text-base text-slate-600 dark:text-zinc-400 max-w-md mx-auto">
                    Section 1 is available as a free preview. To explore all 30 conversational blueprint sections and lessons, unlock full course access.
                  </p>
                </div>
                ...
                <div className="pt-2 flex flex-col sm:flex-row items-center justify-center gap-3">
                  <button
                    type="button"
                    onClick={() => navigate("/courses/basic-eng-vocab/everyday-word/section/1")}
