...
// যেসব ফিল্ড কখনোই সরাসরি টেক্সটবক্সে এডিট করা যাবে না
...
export default function SectionEditor() {
    ...
    // কোর্স আইডি এবং সেকশন নাম্বার নির্ধারণ
    const { targetCourseId, targetSectionNumber } = useMemo(() => {
        if (isCourseCategory) {
            const id = sanitizeId(paramValues[0] || activeCategory?.courseId);
            return { targetCourseId: id, targetSectionNumber: null };
    ...
    useEffect(() => {
        ...
        const fetchDocumentData = async () => {
            ...
            try {
                ...
                // ডেটাকে দুটি ভাগে ভাগ করা: Locked Fields এবং Editable Fields
                const lockedFields = {};
                const editableFields = {};

                Object.keys(fetchedData).forEach((key) => {
                    if (IMMUTABLE_KEYS.includes(key)) {
                        lockedFields[key] = fetchedData[key];
                    } else {
                        editableFields[key] = fetchedData[key];
                    }
                });

                setLockedData(lockedFields);
                setRawJsonText(JSON.stringify(editableFields, null, 2));
            } catch (err) {
                setStatus({
                    message: String(err.response?.data?.message || err.message || 'Failed to fetch document data.'),
                    error: true,
        ...
        fetchDocumentData();
    }, [isCourseCategory, targetCourseId, targetSectionNumber, apiBase, axiosSecure]);
    ...
    const handleSaveToDatabase = async () => {
        ...
        try {
            if (!rawJsonText.trim()) {
                throw new Error('Editable content cannot be empty.');
            }

            let parsedEditable;
            try {
                parsedEditable = safeJsonParse(rawJsonText);
            } catch (parseErr) {
                throw new Error(`Invalid JSON syntax: ${parseErr.message}`);
            }

            // Original key order ebong values thik rekhe merge kora
            // lockedData-te jeta chilo shudhu shetai thakbe, kono extra field add hobena
            const finalPayload = {};

            // 1. Prothome lockedData-r keys gula original order-e assign kora
            Object.keys(lockedData).forEach((key) => {
                finalPayload[key] = lockedData[key];
            });

            // 2. Erpor editable field gula assign kora
            Object.keys(parsedEditable).forEach((key) => {
                finalPayload[key] = parsedEditable[key];
            });

            let saveEndpoint = '';

            if (isCourseCategory) {
                const finalCourseId = sanitizeId(finalPayload.courseId || targetCourseId);
                if (!finalCourseId) {
                    throw new Error('Course identifier is missing.');
                }
                finalPayload.courseId = finalCourseId;
                saveEndpoint = `${apiBase}/${finalCourseId}`;
            } else {
                const finalCourseId = sanitizeId(finalPayload.courseId || targetCourseId);
                const finalSectionNumber = sanitizeId(finalPayload.sectionNumber || targetSectionNumber);

                if (!finalCourseId || !finalSectionNumber) {
                    throw new Error('Course ID or Section Number identifier is missing.');
                }

                finalPayload.courseId = finalCourseId;
                finalPayload.sectionNumber = Number(finalSectionNumber);

                saveEndpoint = `${apiBase}/${finalCourseId}/${finalSectionNumber}`;
            }

            const response = await axiosSecure.put(saveEndpoint, finalPayload);

            await Swal.fire({
                icon: 'success',
                title: 'Saved Successfully!',
                text: response.data?.message || 'Document updated in database.',
                timer: 1500,
                showConfirmButton: false,
            });
    ...
    const headerTitle = isCourseCategory
        ? targetCourseId
        : `Section #${targetSectionNumber} (${targetCourseId})`;
    ...
    return (
        <div style={{ padding: '24px', fontFamily: 'sans-serif', width: '100%', boxSizing: 'border-box' }}>
            ...
            {/* Read-Only / Immutable Fields Meta Panel */}
            {Object.keys(lockedData).length > 0 && (
                <div
                    style={{
                        marginBottom: '16px',
                        padding: '12px 16px',
                ...
                >
                    <div style={{ fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '8px', textTransform: 'uppercase' }}>
                        🔒 Protected Document Attributes (Read-Only)
                    </div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                        {Object.entries(lockedData).map(([key, value]) => (
                            <div
                                key={key}
                                style={{
                                    display: 'flex',
